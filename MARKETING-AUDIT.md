...
| Location | Finding |
|---|---|
| ~~`index.html:707`~~ | "Watch Our Video Demonstration" embedded Vimeo `60616160` — the **RED template's stock demo reel** (same ID appears throughout `_templates/red/`). **Resolved (2026-07-23): the section is now commented out in `index.html` so it no longer displays — hidden, not deleted, so it can be restored with a real video.** Follow-up to swap in a genuine logicacode video is tracked as T4a. |
...
- [~] **T4a · P1 · Homepage video — hidden, awaiting a real one.** The stock Vimeo section (`index.html` section-5) is now **commented out** so it no longer renders — hidden per owner decision, not deleted, markup kept in place for restoration. Verified in headless Chrome: section-5 is absent from the live DOM and no Vimeo request fires. *Remaining:* replace the commented `iframe data-src` with a genuine logicacode video (e.g. from the `@logicacode` YouTube channel) and uncomment the section — or drop it for good. Hand over the video URL and it is a one-line swap.
